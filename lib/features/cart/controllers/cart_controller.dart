...
class CartController extends GetxController implements GetxService {
  ...
  Future<void> addToCartOnline(OnlineCart onlineCart, {CartModel? existCartData, bool fromDirectlyAdd = false}) async {
    ...
    if(response.statusCode == 200) {
      ...
      if(!Get.currentRoute.contains(RouteHelper.restaurant)) {
        ...
        // Only navigate to cart if user is logged in
  ...
  // Check if item can be added to cart based on restaurant and business status
  bool canAddItemToCart(Product product) {
    ...
    if (restaurantController.restaurant != null) {
      if (restaurantController.restaurant!.active == false) {
        showCustomSnackBar('restaurant_temporarily_closed'.tr);
        return false;
      }
  ...
  bool shouldShowClosedOverlay(Product product) {
    // Check if business is shutdown
    bool isBusinessShutdown = Get.find<SplashController>().configModel?.businessShutdown ?? false;
    if (isBusinessShutdown) {
      return true;
  ...
  String getClosedText(Product product) {
    // Check if business is shutdown
    bool isBusinessShutdown = Get.find<SplashController>().configModel?.businessShutdown ?? false;
    if (isBusinessShutdown) {
